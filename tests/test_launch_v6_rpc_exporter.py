"""Adversarial RPC transport/discovery tests; fixtures are local E-10 bytes."""
import base64
import copy
import hashlib
import io
...
from e10_verifier import expand_bundle
from launch_v6_rpc_exporter import Client, MAX_RESPONSE, export, main, pda, read_accounts

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "tools"))
from e10_recorded_rpc import GENESIS, RecordedRpc, manifest_for, serve  # noqa: E402


class V6RpcTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.snapshots = expand_bundle(json.loads((ROOT / "examples/e10_rehearsal_bundle.json").read_text()))
    ...
    def test_export_refuses_previous_layout_and_bad_admission_even_with_matching_rpc_context(self):
        address = self.snapshot['accounts']['withdrawal_0_1']['address']
        original = self.rpc.bank[address]['data_hex']
        for mode in ('old_layout', 'oversize', 'late_start'):
            with self.subTest(mode=mode):
                raw = bytearray.fromhex(original)
                if mode == 'old_layout':
                    del raw[122:138]
                elif mode == 'oversize':
                    struct.pack_into('<q', raw, 130, struct.unpack_from('<q', raw, 122)[0] + 301)
                else:
                    struct.pack_into('<q', raw, 138, struct.unpack_from('<q', raw, 130)[0] + 1)
                self.rpc.bank[address]['data_hex'] = raw.hex()
                with self.assertRaisesRegex(ValueError, 'RPC_WITHDRAWAL_LAYOUT|WITHDRAWAL_ADMISSION'):
    ...
    def test_generic_change_tombstone_included_and_missing_history_rejected(self):
        # Synthetic cancelled generic proposal exercises the v6 discovery path;
        ...
        address, bump = find_program_address((b"launch-v6-change", key, struct.pack("<Q", 1)), _pubkey(self.snapshot["program_id"]))
        raw = hashlib.sha256(b"account:ChangeProposalV6").digest()[:8] + key
    ...
    def test_key_indexes_are_included_in_account_limit(self):
        ...
        for nonce in range(1, 45):
            ...
            address = pda(b"launch-v6-withdraw", _pubkey(policy), b"\x00", struct.pack("<Q", nonce))
    ...
    def test_cli_failure_writes_nothing_and_success_never_overwrites_evidence(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("sys.argv", args), patch("launch_v6_rpc_exporter.Client", return_value=self.rpc), patch("sys.stdout", new_callable=io.StringIO):
