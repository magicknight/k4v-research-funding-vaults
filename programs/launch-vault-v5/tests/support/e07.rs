//! E-07 signed native-loader/SPL setup and beneficiary recovery continuity.
...
pub(super) fn prepare(
    ...
    let target = Keypair::new_from_array([87; 32]);
    assert_eq!(target.pubkey(), ID);
    deploy(f, &target, &artifact("v5-test/launch_vault_v5.so"), true);
    f.run_extra(
        vec![
...
fn snapshot(
    ...
    serde_json::json!({"schema":"K4V-LAUNCH-V5-RAW-SNAPSHOT-v1","program_id":ID.to_string(),
...
fn signed_native_loader_dual_withdrawal_recovery_preserves_money_notices_and_year_boundary() {
    ...
    let new_f = key("e07-founder-successor-symbolic-test-key");
    let new_t = key("e07-treasury-successor-symbolic-test-key");
    ...
    f.time(f.config.t0 + CLIFF + CHANGE_NOTICE - 1);
    f.reject_unchanged(
    ...
    assert_eq!(
        f.read::<TreasuryApprovalV5>(f.approval(9)).author,
        old_t.pubkey()
    );
    f.reject_unchanged(
    ...
    let expiry = f
        .read::<WithdrawalProposalV5>(f.withdrawal_proposal(FOUNDER, 3))
        .expires_at;
    assert_eq!(expiry, f.config.t0 + 13 * PERIOD);
    f.time(expiry);
    f.reject_unchanged(
    ...
    if let Ok(path) = std::env::var("K4V_E07_REHEARSAL_OUT") {
        let value = serde_json::json!({"schema":"K4V-E07-LOCAL-REHEARSAL-v1",
            "scope":"AUTHOR_RUN_LOCAL_LITESVM","private_keys_serialized":false,
