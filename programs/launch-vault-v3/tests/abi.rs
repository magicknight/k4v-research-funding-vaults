use ::launch_vault_v3::{
    identity, period_at, period_start, AnnualRule, LaunchConfig, LaunchPolicyV3, LaunchVaultV3,
    TreasuryApprovalV3, CLIFF, ID, PERIOD,
};
...
fn frozen_identity_vector_matches_compiled_serialization() {
    let v: serde_json::Value = serde_json::from_str(include_str!(
        "../../../spec/LAUNCH_V3_IDENTITY_VECTOR_v1.json"
    ))
    .unwrap();
    let key = |name: &str| {
...
fn committed_idl_discriminators_and_account_sizes_match_compiled_abi() {
    ...
    let idl: serde_json::Value =
        serde_json::from_str(include_str!("../../../idl/launch_vault_v3.json")).unwrap();
    ...
    for (name, discriminator, bytes) in [
        (
            "LaunchPolicyV3",
            LaunchPolicyV3::DISCRIMINATOR,
            LaunchPolicyV3::INIT_SPACE,
        ),
        (
            "LaunchVaultV3",
            LaunchVaultV3::DISCRIMINATOR,
            LaunchVaultV3::INIT_SPACE,
        ),
        (
            "TreasuryApprovalV3",
            TreasuryApprovalV3::DISCRIMINATOR,
            TreasuryApprovalV3::INIT_SPACE,
        ),
    ] {
        let a = idl["accounts"]
            .as_array()
    ...
    assert_eq!(data.len(), 8 + LaunchVaultV3::INIT_SPACE);
}
