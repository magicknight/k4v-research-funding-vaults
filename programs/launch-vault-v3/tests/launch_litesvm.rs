...
use ::launch_vault_v3::{accounts, instruction, *};
...
impl Fixture {
    fn new(disabled: bool, solo: bool) -> Self {
        ...
        let directory = if disabled { "v3-disabled" } else { "v3-test" };
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join(format!("../../target/{directory}/launch_vault_v3.so"));
        ...
        let policy = Pubkey::find_program_address(&[b"launch-v3-policy", &hash], &ID).0;
        let mut f = Self {
            svm,
            creator,
            founder,
    ...
    fn rebind(&mut self) {
        ...
        self.policy = Pubkey::find_program_address(&[b"launch-v3-policy", &self.hash], &ID).0;
    }
    ...
    fn vault(&self, role: u8) -> Pubkey {
        Pubkey::find_program_address(&[b"launch-v3-vault", self.policy.as_ref(), &[role]], &ID).0
    }

    fn vault_token(&self, role: u8) -> Pubkey {
        Pubkey::find_program_address(&[b"launch-v3-token", self.vault(role).as_ref()], &ID).0
    }

    fn approval(&self, period: u64) -> Pubkey {
        Pubkey::find_program_address(
            &[
                b"launch-v3-approval",
                self.policy.as_ref(),
                &period.to_le_bytes(),
            ],
            &ID,
    ...
    fn p(&self) -> LaunchPolicyV3 {
        self.read(self.policy)
    }
    fn v(&self, role: u8) -> LaunchVaultV3 {
...
fn treasury_notice_boundary_recipient_owner_and_consumption_are_enforced() {
    ...
    f.report(3 * UNIT);
    f.reject_unchanged(f.release_ix(TREASURY, UNIT, 1), "InvalidApproval");
    let a: TreasuryApprovalV3 = f.read(f.approval(1));
...
fn failed_token_cpi_rolls_back_policy_vault_and_treasury_approval() {
    ...
    assert_eq!(f.read::<TreasuryApprovalV3>(f.approval(6)).consumed, 0);
    f.conserved();
}
...
fn export_raw_sbf_state_for_independent_verification() {
    ...
    f.conserved();
    if let Ok(path) = std::env::var("K4V_V3_SNAPSHOT_OUT") {
        ...
        let snapshot = serde_json::json!({"schema":"K4V-LAUNCH-V3-RAW-SNAPSHOT-v1",
