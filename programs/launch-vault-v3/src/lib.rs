...
declare_id!("AhTz3JFbaEvk1PMxEsmG89YiTZQJ4ALKoFc8vyr1Cf8m");

#[program]
pub mod launch_vault_v3 {
    use super::*;
    ...
    pub fn open_policy(
        ...
        ctx.accounts.policy.set_inner(LaunchPolicyV3 {
    ...
    pub fn deposit(ctx: Context<Deposit>, role: u8, amount: u64) -> Result<()> {
        ...
        ctx.accounts.vault.set_inner(LaunchVaultV3 {
            policy: policy.key(),
    ...
    pub fn refund(ctx: Context<Refund>) -> Result<()> {
        ...
        let seeds: &[&[u8]] = &[b"launch-v3-vault", policy_key.as_ref(), &role, &bump];
        token::transfer_checked(
            CpiContext::new(
                ctx.accounts.token_program.key(),
                TransferChecked {
    ...
    pub fn approve_treasury(ctx: Context<ApproveTreasury>, period: u64, need: u64) -> Result<()> {
        ...
        ctx.accounts.approval.set_inner(TreasuryApprovalV3 {
            policy: p.key(),
            period,
