use ::launch_vault_v4::{
    identity, period_at, period_start, AnnualRule, ChangeProposalV4, LaunchConfig, LaunchPolicyV4,
    LaunchVaultV4, TreasuryApprovalV4, CLIFF, ID, PERIOD,
};
...
fn frozen_identity_vector_matches_compiled_serialization() {
    let v: serde_json::Value = serde_json::from_str(include_str!(
        "../../../spec/LAUNCH_V4_IDENTITY_VECTOR_v1.json"
    ))
    .unwrap();
    let key = |name: &str| {
...
fn committed_idl_discriminators_and_account_sizes_match_compiled_abi() {
    ...
    let idl: serde_json::Value =
        serde_json::from_str(include_str!("../../../idl/launch_vault_v4.json")).unwrap();
    ...
    for (name, discriminator, bytes) in [
        (
            "ChangeProposalV4",
            ChangeProposalV4::DISCRIMINATOR,
            ChangeProposalV4::INIT_SPACE,
        ),
        (
            "LaunchPolicyV4",
            LaunchPolicyV4::DISCRIMINATOR,
            LaunchPolicyV4::INIT_SPACE,
        ),
        (
            "LaunchVaultV4",
            LaunchVaultV4::DISCRIMINATOR,
            LaunchVaultV4::INIT_SPACE,
        ),
        (
            "TreasuryApprovalV4",
            TreasuryApprovalV4::DISCRIMINATOR,
            TreasuryApprovalV4::INIT_SPACE,
        ),
    ] {
        let a = idl["accounts"]
            .as_array()
    ...
    assert_eq!(data.len(), 8 + LaunchVaultV4::INIT_SPACE);
}
