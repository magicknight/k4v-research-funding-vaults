...
| Signer loss | Multisig operations | MECHANISM ESTABLISHED LOCALLY / NOT DEPLOYED | A Squads 2-of-3 vault PDA satisfies B2's Signer constraint, and a member can be replaced by the survivors with the stored address unchanged, so B2 needs no rotation instruction. No multisig exists yet and the probe's members were all generated by the probe |
| Signer collusion | Independence of the members | OPEN | Survival of key loss is not independence. A committee of keys one person holds is one person; this needs real separate holders, not a contract change |
...
| Oracle deliberately reports zero | Single reporter and fail-closed window | AI PREAUDIT MEDIUM / DESIGN DECISION OPEN | Measure the minimum halt and recovery path; choose quorum/emergency rotation or explicitly retain the 90-day fail-closed power |
