...
export const PROGRAM = new PublicKey('FixSiDfTxvoy5Zgjp5KdFU8U23ChwCxPWY3WTkmMW2fU');
...
export const CODE_SHA256 = '36e5b4f8916d7c4844e625f8665dd91b8057c43e88699168d0cb2936cd02c171';
...
export async function readBoundPolicy(rpc, binding) {
  ...
  requireThat(code.length >= 45 + 529584 && code.length <= 2000000 && code.readUInt32LE(0) === 3 &&
    code[12] === 0 && hash(code.subarray(45, 45 + 529584)) === CODE_SHA256 &&
    code.subarray(45 + 529584).every(b => b === 0) && code.readBigUInt64LE(4) <= BigInt(r.context.slot), 'PROGRAM_CODE');
  requireThat(clock.readBigUInt64LE(0) === BigInt(r.context.slot), 'CLOCK_BANK');
  requireThat(policy.subarray(0, 8).equals(Buffer.from(hash(Buffer.from('account:LaunchPolicyV6')), 'hex').subarray(0, 8)), 'POLICY_DISCRIMINATOR');
  requireThat(policy.subarray(168, 200).toString('hex') === binding.identityHex &&
    ...
    pda(Buffer.from('launch-v6-policy'), policy.subarray(168, 200)).toBase58() === binding.policy, 'POLICY_BINDING');
...
export function proposalInstruction({ payer, initiator, cosigner, successor, policy, role, recovery,
  ...
  const proposal = pda(Buffer.from('launch-v6-withdraw'), policyKey.toBuffer(), Buffer.from([role]), integer(nonce));
  const record = pda(Buffer.from('launch-v6-key'), policyKey.toBuffer(), successorKey.toBuffer());
