"""Independent v5 raw-account decoder, governance reconstruction and accounting.
...
PROGRAM = "EUhWSZAfU8hDki7AXskYrwh8ErwXN8iqicaHTP7yQfYS"
...
def read_policy(snapshot):
    address, r = account(snapshot, "policy", PROGRAM, "LaunchPolicyV5")
    ...
    r.finish()
    preimage = b"k4v-launch-policy-v5-test-profile-1" + _pubkey(PROGRAM)
    preimage += b"".join(_pubkey(p[k]) for k in ("creator", "mint", "founder", "treasury", "initial_oracle"))
    preimage += p["spec_hash"] + struct.pack("<qqQHqBq", CLIFF, PERIOD, 12, 500, 7_776_000, 2, PERIOD) + config_bytes
    require(hashlib.sha256(preimage).digest() == p["identity"], "POLICY_IDENTITY")
    expected, bump = find_program_address((b"launch-v5-policy", p["identity"]), _pubkey(PROGRAM))
...
def read_vault(snapshot, name):
    address, r = account(snapshot, name, PROGRAM, "LaunchVaultV5")
...
def verify_governance(snapshot, p, now):
    ...
    for nonce in range(1, n + 1):
        address, r = account(snapshot, f"change_{nonce}", PROGRAM, "ChangeProposalV5")
        ...
        r.finish()
        expected, bump = find_program_address((b"launch-v5-change", _pubkey(p["address"]),
...
def verify(snapshot):
    require(snapshot["schema"] == "K4V-LAUNCH-V5-RAW-SNAPSHOT-v1" and snapshot["program_id"] == PROGRAM, "SCHEMA_OR_PROGRAM")
    ...
    for i, role in enumerate(("founder", "treasury")):
        ...
        expected, bump = find_program_address((b"launch-v5-vault", _pubkey(p["address"]), bytes([i])), _pubkey(PROGRAM))
        ...
        address, _ = find_program_address((b"launch-v5-token", _pubkey(v["address"])), _pubkey(PROGRAM))
...
def read_approvals(snapshot, p, now):
    ...
    result = []
    for name in names:
        address, r = account(snapshot, name, PROGRAM, 'TreasuryApprovalV5')
        ...
        r.finish()
        expected, bump = find_program_address((b'launch-v5-approval', _pubkey(p['address']), struct.pack('<Q', a['period'])), _pubkey(PROGRAM))
...
def verify_withdrawal(snapshot, p, approvals, now):
    ...
    for role in range(2):
        ...
        for nonce in range(1, state['sequence'] + 1):
            ...
            address, r = account(snapshot, name, PROGRAM, 'WithdrawalProposalV5')
            ...
            r.finish()
            expected, bump = find_program_address((b'launch-v5-withdraw', _pubkey(p['address']), bytes([role]), struct.pack('<Q', nonce)), _pubkey(PROGRAM))
    ...
    for name in snapshot['accounts']:
        if not name.startswith('key_'):
            continue
        address, r = account(snapshot, name, PROGRAM, 'WithdrawalKeyV5')
        k = {'policy': r.key(), 'subject': r.key(), 'history_mask': r.number('B'),
             'pending_mask': r.number('B'), 'recipient': r.number('B'), 'bump': r.number('B')}
        r.finish()
        expected, bump = find_program_address((b'launch-v5-key', _pubkey(p['address']), _pubkey(k['subject'])), _pubkey(PROGRAM))
