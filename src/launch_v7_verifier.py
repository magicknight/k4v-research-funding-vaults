"""Independent v7 raw-account decoder, governance reconstruction and accounting.
...
Use e11b_verifier for additional pinned loader-byte and whole-rehearsal checks.
"""
import argparse
import hashlib
import json
...
PROGRAM = "CYFsfATtQB3Excjsm4Cuh8ZWnPE5j6XAU3GS3RKXmUcK"
...
def read_policy(snapshot):
    address, r = account(snapshot, "policy", PROGRAM, "LaunchPolicyV7")
    ...
    expected, bump = find_program_address((b"launch-v7-policy", p["identity"]), _pubkey(PROGRAM))
...
def read_vault(snapshot, name):
    address, r = account(snapshot, name, PROGRAM, "LaunchVaultV7")
...
def verify_governance(snapshot, p, now):
    ...
    for nonce in range(1, n + 1):
        address, r = account(snapshot, f"change_{nonce}", PROGRAM, "ChangeProposalV7")
        ...
        r.finish()
        expected, bump = find_program_address((b"launch-v7-change", _pubkey(p["address"]),
...
def verify(snapshot):
    require(snapshot["schema"] == "K4V-LAUNCH-V7-RAW-SNAPSHOT-v1" and snapshot["program_id"] == PROGRAM, "SCHEMA_OR_PROGRAM")
    ...
    for i, role in enumerate(("founder", "treasury")):
        ...
        expected, bump = find_program_address((b"launch-v7-vault", _pubkey(p["address"]), bytes([i])), _pubkey(PROGRAM))
        ...
        address, _ = find_program_address((b"launch-v7-token", _pubkey(v["address"])), _pubkey(PROGRAM))
...
def read_approvals(snapshot, p, now):
    ...
    result = []
    for name in names:
        address, r = account(snapshot, name, PROGRAM, 'TreasuryApprovalV7')
        ...
        r.finish()
        expected, bump = find_program_address((b'launch-v7-approval', _pubkey(p['address']), struct.pack('<Q', a['period'])), _pubkey(PROGRAM))
...
def verify_withdrawal(snapshot, p, approvals, now):
    ...
    for role in range(2):
        ...
        for nonce in range(1, state['sequence'] + 1):
            ...
            address, r = account(snapshot, name, PROGRAM, 'WithdrawalProposalV7')
            ...
            r.finish()
            expected, bump = find_program_address((b'launch-v7-withdraw', _pubkey(p['address']), bytes([role]), struct.pack('<Q', nonce)), _pubkey(PROGRAM))
    ...
    for name in snapshot['accounts']:
        if not name.startswith('key_'):
            continue
        address, r = account(snapshot, name, PROGRAM, 'WithdrawalKeyV7')
        k = {'policy': r.key(), 'subject': r.key(), 'history_mask': r.number('B'),
             'pending_mask': r.number('B'), 'recipient': r.number('B'), 'bump': r.number('B')}
        r.finish()
        expected, bump = find_program_address((b'launch-v7-key', _pubkey(p['address']), _pubkey(k['subject'])), _pubkey(PROGRAM))
