"""E-11B raw-account, pinned immutable local loader and continuity verification.
...
from launch_v7_verifier import PROGRAM, verify, read_policy, require
...
BUILD = json.loads((Path(__file__).resolve().parents[1] / 'spec/LAUNCH_V7_BUILD_IDENTITY_v1.json').read_text())
...
def expand_bundle(bundle):
    require(bundle['schema'] == 'K4V-E11B-REHEARSAL-BUNDLE-v1', 'BUNDLE_SCHEMA')
...
def verify_bundle(bundle):
    ...
    # Reconstruct the delayed admissions from raw proposal bytes. Receipt labels
    # alone cannot establish that the actual start is later than the lower bound.
    admitted = []
    for role in range(2):
        raw = bytes.fromhex(by['recovery_pending']['accounts'][f'withdrawal_{role}_1']['data_hex'])
        valid_from, valid_until, created_at, execute_after, expires_at = struct.unpack_from('<qqqqq', raw, 122)
        require(created_at - valid_from == 30 and valid_until - valid_from == 300,
                'REHEARSAL_DELAYED_ADMISSION')
        require(execute_after - created_at == 7_776_000 and expires_at - execute_after == 2_592_000,
                'REHEARSAL_FULL_NOTICE')
        admitted.append({'role': role, 'delay_seconds': 30, 'created_at': created_at})
    ...
    return {'valid': True, 'checkpoints_verified': len(snapshots), 'results': results,
            'delayed_admissions_from_supplied_raw_bytes': admitted,
