"""Read-only v5 export: explicit identity, complete history, one final bank.
...
from e07_verifier import CLOCK, LOADER, SYSVAR, verify_graph
from launch_v5_verifier import PROGRAM, account, read_policy, require

MAX_RESPONSE = 3_000_000
MAX_ACCOUNTS = 100
MANIFEST_SCHEMA = "K4V-V5-RPC-REVIEW-MANIFEST-v1"
...
def validate_manifest(manifest):
    ...
    require(expected["policy"] == pda(b"launch-v5-policy", bytes.fromhex(expected["identity_sha256"])), "RPC_EXPECTED_POLICY_PDA")
    ...
    for role, name in enumerate(("founder", "treasury")):
        vault = pda(b"launch-v5-vault", policy_key, bytes([role]))
        addresses[name + "_vault"] = vault
        addresses[name + "_token"] = pda(b"launch-v5-token", _pubkey(vault))
...
def read_accounts(client, addresses, minimum):
    ...
    return {"schema": "K4V-LAUNCH-V5-RAW-SNAPSHOT-v1", "program_id": PROGRAM,
...
def history_addresses(p, periods):
    ...
    result = {f"change_{n}": pda(b"launch-v5-change", key, struct.pack("<Q", n))
              for n in range(1, sequences[0] + 1)}
    for role in range(2):
        result.update({f"withdrawal_{role}_{n}": pda(b"launch-v5-withdraw", key, bytes([role]), struct.pack("<Q", n))
                       for n in range(1, sequences[role + 1] + 1)})
    result.update({f"approval_{n}": pda(b"launch-v5-approval", key, struct.pack("<Q", n)) for n in periods})
    return result
...
def key_addresses(discovery, history, policy):
    ...
    for name in history:
        if name.startswith("withdrawal_"):
            _, r = account(discovery, name, PROGRAM, "WithdrawalProposalV5")
            require(len(r.data) == 156, "RPC_WITHDRAWAL_LAYOUT")
            subjects.add(_base58_encode(r.data[89:121]))
        elif name.startswith("approval_"):
            _, r = account(discovery, name, PROGRAM, "TreasuryApprovalV5")
            require(len(r.data) == 177, "RPC_APPROVAL_LAYOUT")
            subjects.add(_base58_encode(r.data[80:112]))
    return {"key_" + k: pda(b"launch-v5-key", _pubkey(policy), _pubkey(k)) for k in sorted(subjects)}
...
def export(client, manifest, min_context_slot=0):
    ...
    return {"schema": "K4V-V5-RPC-OBSERVATION-v1", "snapshot": final, "verification": verification,
