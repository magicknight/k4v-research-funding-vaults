"""Read-only v6 export: explicit identity, complete history, one final bank.
...
from e10_verifier import CLOCK, LOADER, SYSVAR, verify_graph
from launch_v6_verifier import PROGRAM, account, read_policy, require

MAX_RESPONSE = 3_000_000
MAX_ACCOUNTS = 100
MANIFEST_SCHEMA = "K4V-V6-RPC-REVIEW-MANIFEST-v1"
...
def validate_manifest(manifest):
    ...
    require(expected["policy"] == pda(b"launch-v6-policy", bytes.fromhex(expected["identity_sha256"])), "RPC_EXPECTED_POLICY_PDA")
    ...
    for role, name in enumerate(("founder", "treasury")):
        vault = pda(b"launch-v6-vault", policy_key, bytes([role]))
        addresses[name + "_vault"] = vault
        addresses[name + "_token"] = pda(b"launch-v6-token", _pubkey(vault))
...
def read_accounts(client, addresses, minimum):
    ...
    return {"schema": "K4V-LAUNCH-V6-RAW-SNAPSHOT-v1", "program_id": PROGRAM,
...
def history_addresses(p, periods):
    ...
    result = {f"change_{n}": pda(b"launch-v6-change", key, struct.pack("<Q", n))
              for n in range(1, sequences[0] + 1)}
    for role in range(2):
        result.update({f"withdrawal_{role}_{n}": pda(b"launch-v6-withdraw", key, bytes([role]), struct.pack("<Q", n))
                       for n in range(1, sequences[role + 1] + 1)})
    result.update({f"approval_{n}": pda(b"launch-v6-approval", key, struct.pack("<Q", n)) for n in periods})
    return result
...
def export(client, manifest, min_context_slot=0):
    ...
    return {"schema": "K4V-V6-RPC-OBSERVATION-v1", "snapshot": final, "verification": verification,
