...
PROGRAM = ROOT / "candidates/launch-vault-v7"
...
def build():
    ...
    assert len(idl["instructions"]) == 19
    assert len(idl["accounts"]) == 7
...
if __name__ == "__main__":
    ...
    text = build()
    destination = ROOT / "idl/launch_vault_v7.json"
    if args.check:
        assert destination.read_text() == text, "Committed IDL differs from compiler output"
        print("Launch v7 compiler-generated IDL: PASS")
    else:
