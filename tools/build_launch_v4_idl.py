...
PROGRAM = ROOT / "programs/launch-vault-v4"
...
def build():
    ...
    assert len(idl["instructions"]) == 13
    assert len(idl["accounts"]) == 4
...
if __name__ == "__main__":
    ...
    text = build()
    destination = ROOT / "idl/launch_vault_v4.json"
    if args.check:
        assert destination.read_text() == text, "Committed IDL differs from compiler output"
        print("Launch v4 compiler-generated IDL: PASS")
    else:
