...
PROGRAM = ROOT / "candidates/launch-vault-v6"
...
def build():
    ...
    assert len(idl["instructions"]) == 18
    assert len(idl["accounts"]) == 6
...
if __name__ == "__main__":
    ...
    text = build()
    destination = ROOT / "idl/launch_vault_v6.json"
    if args.check:
        assert destination.read_text() == text, "Committed IDL differs from compiler output"
        print("Launch v6 compiler-generated IDL: PASS")
    else:
