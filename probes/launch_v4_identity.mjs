...
export function encodeConfig(config) {
  exact(config, [...fields, 'annual_rules', 'recovery_keys']);
  ...
  if (!Array.isArray(config.recovery_keys) || config.recovery_keys.length !== 3) throw new Error('Exactly three recovery keys required');
  config.recovery_keys.forEach((key, i) => {
    if (typeof key !== 'string' || !/^[0-9a-f]{64}$/.test(key)) throw new Error('Recovery key must be 32 hex bytes');
    Buffer.from(key, 'hex').copy(bytes, 204 + 32 * i);
  });
  return bytes;
}
...
export function boundLaunchIdentity({ program, creator, mint, founder, treasury, oracle, specHash, config }) {
  ...
  const constants = Buffer.alloc(35);
  ...
  return createHash('sha256').update(Buffer.concat([
    Buffer.from('k4v-launch-policy-v4-test-profile-1'), ...keys, constants, encodeConfig(config),
  ])).digest();
}
