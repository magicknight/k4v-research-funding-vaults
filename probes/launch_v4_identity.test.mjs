...
import { boundLaunchIdentity, encodeConfig } from './launch_v4_identity.mjs';

const v = JSON.parse(readFileSync(new URL('../spec/LAUNCH_V4_IDENTITY_VECTOR_v1.json', import.meta.url)));
...
test('actors, every base field, annual rule and source hash are identity-bound', () => {
  ...
  for (const key of Object.keys(input.config).filter(key => !['annual_rules', 'recovery_keys'].includes(key))) {
...
test('all recovery keys and their order are identity-bound', () => {
  for (let i = 0; i < 3; i++) {
    const config = structuredClone(input.config);
    config.recovery_keys[i] = 'ee'.repeat(32);
    assert.notEqual(boundLaunchIdentity({ ...input, config }).toString('hex'), v.identity_hex);
  }
  const config = structuredClone(input.config);
  config.recovery_keys.reverse();
  assert.notEqual(boundLaunchIdentity({ ...input, config }).toString('hex'), v.identity_hex);
  assert.throws(() => encodeConfig({ ...input.config, recovery_keys: [] }));
  assert.throws(() => encodeConfig({ ...input.config, recovery_keys: ['bad', ...input.config.recovery_keys.slice(1)] }));
});
