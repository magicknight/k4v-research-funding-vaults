...
fn quorum(p: &LaunchPolicyV6, first: Pubkey, second: Pubkey) -> bool {
    first != second
...
pub fn propose(ctx: Context<ProposeChange>, kind: u8, recovery: bool, nonce: u64) -> Result<()> {
    ...
    let execute_after = now
        ...
        .ok_or(LaunchError::Overflow)?;
    ctx.accounts.proposal.set_inner(ChangeProposalV6 {
        policy: p.key(),
        nonce,
        kind,
        recovery,
...
fn pending(p: &LaunchPolicyV6, q: &ChangeProposalV6) -> Result<()> {
    require!(p.state != CANCELLED, LaunchError::InvalidState);
    require!(
