...
declare_id!("FixSiDfTxvoy5Zgjp5KdFU8U23ChwCxPWY3WTkmMW2fU");

#[program]
pub mod launch_vault_v6 {
    use super::*;
    ...
    pub fn open_policy(
        ...
        ctx.accounts.policy.set_inner(LaunchPolicyV6 {
    ...
    pub fn deposit(ctx: Context<Deposit>, role: u8, amount: u64) -> Result<()> {
        ...
        ctx.accounts.vault.set_inner(LaunchVaultV6 {
            policy: policy.key(),
    ...
    pub fn refund(ctx: Context<Refund>) -> Result<()> {
        ...
        let seeds: &[&[u8]] = &[b"launch-v6-vault", policy_key.as_ref(), &role, &bump];
        token::transfer_checked(
            CpiContext::new(
                ctx.accounts.token_program.key(),
                TransferChecked {
    ...
    pub fn approve_treasury(
        ...
        ctx.accounts.approval.set_inner(TreasuryApprovalV6 {
            policy: p.key(),
            period,
