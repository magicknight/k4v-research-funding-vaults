use ::launch_vault_v6::{
    identity, period_at, period_start, AnnualRule, ChangeProposalV6, LaunchConfig, LaunchPolicyV6,
    LaunchVaultV6, TreasuryApprovalV6, WithdrawalKeyV6, WithdrawalProposalV6, CLIFF, ID, PERIOD,
};
...
fn committed_idl_discriminators_and_account_sizes_match_compiled_abi() {
    ...
    let idl: serde_json::Value =
        serde_json::from_str(include_str!("../../../idl/launch_vault_v6.json")).unwrap();
    ...
    for (name, discriminator, bytes) in [
        (
            "WithdrawalKeyV6",
            WithdrawalKeyV6::DISCRIMINATOR,
            WithdrawalKeyV6::INIT_SPACE,
        ),
        (
            "WithdrawalProposalV6",
            WithdrawalProposalV6::DISCRIMINATOR,
            WithdrawalProposalV6::INIT_SPACE,
        ),
        (
            "ChangeProposalV6",
            ChangeProposalV6::DISCRIMINATOR,
            ChangeProposalV6::INIT_SPACE,
        ),
        (
            "LaunchPolicyV6",
            LaunchPolicyV6::DISCRIMINATOR,
            LaunchPolicyV6::INIT_SPACE,
        ),
        (
            "LaunchVaultV6",
            LaunchVaultV6::DISCRIMINATOR,
            LaunchVaultV6::INIT_SPACE,
        ),
        (
            "TreasuryApprovalV6",
            TreasuryApprovalV6::DISCRIMINATOR,
            TreasuryApprovalV6::INIT_SPACE,
        ),
    ] {
        let a = idl["accounts"]
            .as_array()
    ...
    assert_eq!(data.len(), 8 + LaunchVaultV6::INIT_SPACE);
}
