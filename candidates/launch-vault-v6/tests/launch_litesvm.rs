...
impl Fixture {
    ...
    fn new_mode(disabled: bool, solo: bool, native_loader: bool) -> Self {
        ...
        let directory = if disabled { "v6-disabled" } else { "v6-test" };
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join(format!("../../target/{directory}/launch_vault_v6.so"));
        if !native_loader {
        ...
        let policy = Pubkey::find_program_address(&[b"launch-v6-policy", &hash], &ID).0;
        let mut f = Self {
            svm,
            creator,
            founder,
        ...
        f.time(START);
        if native_loader {
            e10::prepare(&mut f, &mint_key, &source_key, &founder_key, &recipient_key);
        }
        f
    }
    ...
    fn run(&mut self, instruction: Instruction) -> Outcome {
        let opening = instruction.program_id == ID
            && instruction
                .data
                .starts_with(instruction::OpenPolicy::DISCRIMINATOR);
    ...
    fn rebind(&mut self) {
        ...
        self.policy = Pubkey::find_program_address(&[b"launch-v6-policy", &self.hash], &ID).0;
    }
    ...
    fn vault(&self, role: u8) -> Pubkey {
        Pubkey::find_program_address(&[b"launch-v6-vault", self.policy.as_ref(), &[role]], &ID).0
    }

    fn vault_token(&self, role: u8) -> Pubkey {
        Pubkey::find_program_address(&[b"launch-v6-token", self.vault(role).as_ref()], &ID).0
    }

    fn approval(&self, period: u64) -> Pubkey {
        Pubkey::find_program_address(
            &[
                b"launch-v6-approval",
                self.policy.as_ref(),
                &period.to_le_bytes(),
            ],
            &ID,
    ...
    fn p(&self) -> LaunchPolicyV6 {
        self.read(self.policy)
    }
    fn v(&self, role: u8) -> LaunchVaultV6 {
...
fn treasury_notice_boundary_recipient_owner_and_consumption_are_enforced() {
    ...
    f.report(3 * UNIT);
    f.reject_unchanged(f.release_ix(TREASURY, UNIT, 1), "InvalidApproval");
    let a: TreasuryApprovalV6 = f.read(f.approval(1));
...
fn failed_token_cpi_rolls_back_policy_vault_and_treasury_approval() {
    ...
    assert_eq!(f.read::<TreasuryApprovalV6>(f.approval(6)).consumed, 0);
    f.conserved();
}
...
fn recovery_quorum_atomically_evicts_hostile_pending_proposal_and_can_cancel_recovery() {
    ...
    assert_eq!(
        f.read::<ChangeProposalV6>(proposal(&f, 1)).status,
        PROPOSAL_CANCELLED
    );
    assert!(f.run(execute(&f, 1)).is_err());
...
fn recovery_clock_rollback_and_cross_policy_execution_are_rejected_atomically() {
    ...
    assert_eq!(f.read::<LaunchPolicyV6>(old_policy).oracle_epoch, 0);
}
...
impl Fixture {
    fn key_record(&self, subject: Pubkey) -> Pubkey {
        Pubkey::find_program_address(
            &[b"launch-v6-key", self.policy.as_ref(), subject.as_ref()],
            &ID,
        )
        .0
    }
    ...
    fn execute_withdrawal_ix(&self, role: u8, nonce: u64, expiry: bool) -> Instruction {
        ...
        let q: WithdrawalProposalV6 = self.read(proposal);
...
fn proposal(f: &Fixture, nonce: u64) -> Pubkey {
    Pubkey::find_program_address(
        &[b"launch-v6-change", f.policy.as_ref(), &nonce.to_le_bytes()],
        &ID,
    )
    .0
}
...
fn counters(p: &LaunchPolicyV6) -> Vec<u64> {
    vec![
        p.period,
        p.shared_used,
