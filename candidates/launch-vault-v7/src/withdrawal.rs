...
pub fn prepare_key(ctx: Context<PrepareWithdrawalKey>, subject: Pubkey) -> Result<()> {
    ...
    ctx.accounts.record.set_inner(WithdrawalKeyV7 {
...
fn quorum(p: &LaunchPolicyV7, role: u8, a: Pubkey, b: Pubkey) -> Result<bool> {
    let keys = p.withdrawal_keys(role)?;
    Ok(a != b && keys.contains(&a) && keys.contains(&b))
}

pub fn eligible_recipient(p: &LaunchPolicyV7, r: &WithdrawalKeyV7) -> Result<()> {
    require!(
        r.subject != p.founder
...
fn eligible_successor(p: &LaunchPolicyV7, role: u8, r: &WithdrawalKeyV7) -> Result<()> {
    require!(
...
pub fn propose(
    ...
    let expires_at = execute_after
        ...
        .ok_or(LaunchError::Overflow)?;
    ctx.accounts.proposal.set_inner(WithdrawalProposalV7 {
        policy: p.key(),
        role,
        nonce,
        epoch,
...
fn pending(p: &LaunchPolicyV7, q: &WithdrawalProposalV7, k: &WithdrawalKeyV7) -> Result<()> {
...
fn finish(
    p: &mut LaunchPolicyV7,
    q: &mut WithdrawalProposalV7,
    k: &mut WithdrawalKeyV7,
    status: u8,
    now: i64,
) {
