...
declare_id!("CYFsfATtQB3Excjsm4Cuh8ZWnPE5j6XAU3GS3RKXmUcK");

#[program]
pub mod launch_vault_v7 {
    use super::*;

    /// Records immutable intent only; no policy or token rights exist yet.
    ...
    pub fn open_prepared_policy(ctx: Context<OpenPreparedPolicy>) -> Result<()> {
        ...
        require!(
            preparation.config.recovery_keys
                == [
        ...
        ctx.accounts.policy.set_inner(LaunchPolicyV7 {
            creator: ctx.accounts.creator.key(),
            mint: ctx.accounts.mint.key(),
            founder: ctx.accounts.founder.key(),
            treasury: ctx.accounts.treasury.key(),
    ...
    pub fn deposit(ctx: Context<Deposit>, role: u8, amount: u64) -> Result<()> {
        ...
        ctx.accounts.vault.set_inner(LaunchVaultV7 {
            policy: policy.key(),
    ...
    pub fn refund(ctx: Context<Refund>) -> Result<()> {
        ...
        let seeds: &[&[u8]] = &[b"launch-v7-vault", policy_key.as_ref(), &role, &bump];
        token::transfer_checked(
            CpiContext::new(
                ctx.accounts.token_program.key(),
                TransferChecked {
    ...
    pub fn approve_treasury(
        ...
        ctx.accounts.approval.set_inner(TreasuryApprovalV7 {
            policy: p.key(),
            period,
    ...
    #[allow(clippy::too_many_arguments)] // Every intent field is in the signed message.
