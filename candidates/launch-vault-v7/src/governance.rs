...
fn quorum(p: &LaunchPolicyV7, first: Pubkey, second: Pubkey) -> bool {
    first != second
...
pub fn propose(ctx: Context<ProposeChange>, kind: u8, recovery: bool, nonce: u64) -> Result<()> {
    ...
    let execute_after = now
        ...
        .ok_or(LaunchError::Overflow)?;
    ctx.accounts.proposal.set_inner(ChangeProposalV7 {
        policy: p.key(),
        nonce,
        kind,
        recovery,
...
fn pending(p: &LaunchPolicyV7, q: &ChangeProposalV7) -> Result<()> {
    require!(p.state != CANCELLED, LaunchError::InvalidState);
    require!(
