...
use ::launch_vault_v7::{accounts, instruction, *};
use anchor_lang::{
...
impl Fixture {
    ...
    fn new_mode(disabled: bool, solo: bool, native_loader: bool) -> Self {
        ...
        let directory = if disabled { "v7-disabled" } else { "v7-test" };
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join(format!("../../target/{directory}/launch_vault_v7.so"));
        if !native_loader {
        ...
        let config = LaunchConfig {
            ...
            founder_recovery_keys: backups[0].each_ref().map(|k| k.pubkey()),
            treasury_recovery_keys: backups[1].each_ref().map(|k| k.pubkey()),
            annual_rules: [
                AnnualRule {
                    start_period: 0,
                    end_period: 12,
        ...
        let policy = Pubkey::find_program_address(&[b"launch-v7-policy", &hash], &ID).0;
        let mut f = Self {
            svm,
            creator,
            founder,
        ...
        f.time(START);
        if native_loader {
            e11b::prepare(&mut f, &mint_key, &source_key, &founder_key, &recipient_key);
        }
        f
    }

    fn run(&mut self, instruction: Instruction) -> Outcome {
        let opening = instruction.program_id == ID
            && instruction
                .data
                .starts_with(instruction::OpenPreparedPolicy::DISCRIMINATOR);
        ...
        if opening && result.is_ok() {
            for subject in [
                self.outsider.pubkey(),
                self.founder.pubkey(),
                self.treasury.pubkey(),
            ] {
                self.prepare_key(subject);
            }
        }
        result
    }
    ...
    fn run_extra(
        ...
        self.run_extra_delayed(instructions, payer, extra, 0)
    }

    fn run_extra_delayed(
        &mut self,
        ...
        .into_iter()
        .chain(self.backups.iter().flatten())
        ...
        // Advance only after the complete message is signed; keep blockhash valid
        // to isolate application timing from blockhash lifetime in this local bank.
        if delay != 0 {
            let before = self.svm.get_sysvar::<Clock>().unix_timestamp;
            self.time(before.checked_add(delay).unwrap());
        }
        self.sent += 1;
    ...
    fn rebind(&mut self) {
        ...
        self.policy = Pubkey::find_program_address(&[b"launch-v7-policy", &self.hash], &ID).0;
    }
    ...
    fn vault(&self, role: u8) -> Pubkey {
        Pubkey::find_program_address(&[b"launch-v7-vault", self.policy.as_ref(), &[role]], &ID).0
    }

    fn vault_token(&self, role: u8) -> Pubkey {
        Pubkey::find_program_address(&[b"launch-v7-token", self.vault(role).as_ref()], &ID).0
    }

    fn approval(&self, period: u64) -> Pubkey {
        Pubkey::find_program_address(
            &[
                b"launch-v7-approval",
                self.policy.as_ref(),
                &period.to_le_bytes(),
            ],
            &ID,
    ...
    fn approve_ix(&self, period: u64, need: u64, recipient: Pubkey) -> Instruction {
        ix(
            accounts::ApproveTreasury {
                ...
                system_program: solana_system_interface::program::ID,
                recipient_record: self.key_record(
                    SplAccount::unpack(&self.svm.get_account(&recipient).unwrap().data)
                        .unwrap()
                        .owner,
                ),
            },
            instruction::ApproveTreasury {
                period,
                need,
                authority_epoch: self.p().withdrawal[1].epoch,
            },
        )
    }
    ...
    fn release_ix(&self, role: u8, amount: u64, approval_period: u64) -> Instruction {
        ix(
            accounts::Release {
                ...
                token_program: TOKEN_ID,
                recipient_record: if role == FOUNDER {
                    None
                } else {
                    Some(
                        self.key_record(
                            SplAccount::unpack(
                                &self.svm.get_account(&self.recipient).unwrap().data,
                            )
                            .unwrap()
                            .owner,
                        ),
            ...
            instruction::Release {
                amount,
                authority_epoch: self.p().withdrawal[role as usize].epoch,
            },
        )
    }
    ...
    fn p(&self) -> LaunchPolicyV7 {
        self.read(self.policy)
    }
    fn v(&self, role: u8) -> LaunchVaultV7 {
...
fn identity_prevents_foreign_creator_and_changed_t0_squatting() {
    ...
    f.run_many(vec![f.prepare_ix()], f.creator.pubkey())
        .unwrap();
...
fn deposits_require_revoked_authorities_exact_amount_and_designated_role() {
    ...
    f.reject_unchanged(wrong, "WithdrawalAuthority");
...
fn treasury_notice_boundary_recipient_owner_and_consumption_are_enforced() {
    ...
    f.report(3 * UNIT);
    f.reject_unchanged(f.release_ix(TREASURY, UNIT, 1), "InvalidApproval");
    let a: TreasuryApprovalV7 = f.read(f.approval(1));
...
fn treasury_cannot_approve_self_current_period_or_replace_a_budget() {
    ...
    f.svm
        ...
        .unwrap();
    f.reject_unchanged(f.approve_ix(1, UNIT, self_account), "KnownSelfPayment");
...
fn failed_token_cpi_rolls_back_policy_vault_and_treasury_approval() {
    ...
    assert_eq!(f.read::<TreasuryApprovalV7>(f.approval(6)).consumed, 0);
    f.conserved();
}
...
fn active_refund_wrong_founder_destination_and_role_swap_are_rejected() {
    ...
    f.reject_unchanged(swapped, "WithdrawalAuthority");
    f.conserved();
}
...
fn annual_inputs_are_immutable_bound_and_reject_invalid_ranges_rates_or_sources() {
    ...
    f.reject_unchanged(f.prepare_ix(), "IdentityMismatch");
    f.rebind();
    f.config.annual_rules[0].release_bps = 501;
    f.reject_unchanged(f.prepare_ix(), "InvalidAnnualRule");
...
fn recovery_quorum_atomically_evicts_hostile_pending_proposal_and_can_cancel_recovery() {
    ...
    assert_eq!(
        f.read::<ChangeProposalV7>(proposal(&f, 1)).status,
        PROPOSAL_CANCELLED
    );
    assert!(f.run(execute(&f, 1)).is_err());
...
fn recovery_clock_rollback_and_cross_policy_execution_are_rejected_atomically() {
    ...
    assert_eq!(f.read::<LaunchPolicyV7>(old_policy).oracle_epoch, 0);
}
...
impl Fixture {
    fn key_record(&self, subject: Pubkey) -> Pubkey {
        Pubkey::find_program_address(
            &[b"launch-v7-key", self.policy.as_ref(), subject.as_ref()],
            &ID,
        )
        .0
    }
    fn prepare_key_ix(&self, subject: Pubkey) -> Instruction {
        ix(
            accounts::PrepareWithdrawalKey {
                payer: self.outsider.pubkey(),
                policy: self.policy,
                record: self.key_record(subject),
                system_program: solana_system_interface::program::ID,
            },
            instruction::PrepareWithdrawalKey { subject },
        )
    }
    fn prepare_key(&mut self, subject: Pubkey) {
        if self.svm.get_account(&self.key_record(subject)).is_none() {
            self.run_many(vec![self.prepare_key_ix(subject)], self.outsider.pubkey())
                .unwrap();
        }
    }
    fn withdrawal_proposal(&self, role: u8, nonce: u64) -> Pubkey {
        Pubkey::find_program_address(
            &[
                b"launch-v7-withdraw",
                self.policy.as_ref(),
                &[role],
                &nonce.to_le_bytes(),
            ],
    ...
    fn propose_withdrawal_ix(&self, role: u8, successor: Pubkey, recovery: bool) -> Instruction {
        let r = self.p().withdrawal[role as usize];
        let keys = if recovery {
            [
                self.backups[role as usize][0].pubkey(),
                self.backups[role as usize][1].pubkey(),
            ]
        } else {
            [r.current, r.current]
        };
        ix(
            accounts::ProposeWithdrawal {
                ...
                successor,
                policy: self.policy,
                proposal: self.withdrawal_proposal(role, r.sequence + 1),
                successor_record: self.key_record(successor),
                system_program: solana_system_interface::program::ID,
            },
            instruction::ProposeWithdrawal {
                ...
                nonce: r.sequence + 1,
                epoch: r.epoch,
                valid_from: self.svm.get_sysvar::<Clock>().unix_timestamp,
                valid_until: self.svm.get_sysvar::<Clock>().unix_timestamp + MAX_SUBMISSION_WINDOW,
                predecessor: r.current,
            },
        )
    }
    fn execute_withdrawal_ix(&self, role: u8, nonce: u64, expiry: bool) -> Instruction {
        let proposal = self.withdrawal_proposal(role, nonce);
        let q: WithdrawalProposalV7 = self.read(proposal);
        let accounts = accounts::ExecuteWithdrawal {
            policy: self.policy,
            proposal,
            successor_record: self.key_record(q.successor),
        };
        if expiry {
            ix(accounts, instruction::ExpireWithdrawal {})
        } else {
            ix(accounts, instruction::ExecuteWithdrawal {})
        }
    }
    fn cancel_withdrawal_ix(&self, role: u8, nonce: u64, a: Pubkey, b: Pubkey) -> Instruction {
        let proposal = self.withdrawal_proposal(role, nonce);
        let q: WithdrawalProposalV7 = self.read(proposal);
        ix(
    ...
    fn propose_withdrawal(&mut self, role: u8, successor: &Keypair, recovery: bool) {
        self.prepare_key(successor.pubkey());
        self.run_extra(
            vec![self.propose_withdrawal_ix(role, successor.pubkey(), recovery)],
            self.outsider.pubkey(),
            &[successor],
        )
        .unwrap();
...
#[path = "support/withdrawal_tests.rs"]
mod withdrawal_tests;

fn proposal(f: &Fixture, nonce: u64) -> Pubkey {
    Pubkey::find_program_address(
        &[b"launch-v7-change", f.policy.as_ref(), &nonce.to_le_bytes()],
        &ID,
    )
    .0
}
...
fn counters(p: &LaunchPolicyV7) -> Vec<u64> {
    vec![
        p.period,
        p.shared_used,
