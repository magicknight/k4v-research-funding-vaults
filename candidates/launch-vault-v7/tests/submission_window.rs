...
//! E-11B signed admission-window probes on the isolated v7 candidate.
...
impl Probe {
    fn new() -> Self {
        ...
        let artifact = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../target/v7-test/launch_vault_v7.so");
        ...
        let policy = Pubkey::find_program_address(&[b"launch-v7-policy", &identity], &ID).0;
        ...
        .0;
        let preparation =
            Pubkey::find_program_address(&[b"launch-v7-preparation", &identity], &ID).0;
        let prepare = ix(
...
fn receipt(name: &str, cases: Vec<serde_json::Value>) {
    if let Ok(directory) = std::env::var("K4V_E11B_PROBE_OUT_DIR") {
        let path = PathBuf::from(directory);
        std::fs::create_dir_all(&path).unwrap();
        let value = serde_json::json!({"schema":"K4V-E11B-V7-SUBMISSION-PROBE-v1", "probe":name,
            "scope":"SIGNED_LOCAL_LITESVM_ON_V7_TEST_SBF", "program_id":ID.to_string(),
...
fn proposal(p: &Probe, role: u8) -> WithdrawalProposalV7 {
    let a = p.svm.get_account(&p.proposal(role)).unwrap();
    assert_eq!(a.data.len(), 172);
    WithdrawalProposalV7::try_deserialize(&mut a.data.as_slice()).unwrap()
}
