use ::launch_vault_v7::{
    identity, period_at, period_start, AnnualRule, ChangeProposalV7, LaunchConfig, LaunchPolicyV7,
    LaunchPreparationV7, LaunchVaultV7, TreasuryApprovalV7, WithdrawalKeyV7, WithdrawalProposalV7,
    CLIFF, ID, PERIOD,
};
...
fn frozen_identity_vector_matches_compiled_serialization() {
    let v: serde_json::Value = serde_json::from_str(include_str!(
        "../../../spec/LAUNCH_V7_IDENTITY_VECTOR_v1.json"
    ))
    .unwrap();
    let key = |name: &str| {
...
fn committed_idl_discriminators_and_account_sizes_match_compiled_abi() {
    ...
    let idl: serde_json::Value =
        serde_json::from_str(include_str!("../../../idl/launch_vault_v7.json")).unwrap();
    ...
    for (name, discriminator, bytes) in [
        (
            "LaunchPreparationV7",
            LaunchPreparationV7::DISCRIMINATOR,
            LaunchPreparationV7::INIT_SPACE,
        ),
        (
            "WithdrawalKeyV7",
            WithdrawalKeyV7::DISCRIMINATOR,
            WithdrawalKeyV7::INIT_SPACE,
        ),
        (
            "WithdrawalProposalV7",
            WithdrawalProposalV7::DISCRIMINATOR,
            WithdrawalProposalV7::INIT_SPACE,
        ),
        (
            "ChangeProposalV7",
            ChangeProposalV7::DISCRIMINATOR,
            ChangeProposalV7::INIT_SPACE,
        ),
        (
            "LaunchPolicyV7",
            LaunchPolicyV7::DISCRIMINATOR,
            LaunchPolicyV7::INIT_SPACE,
        ),
        (
            "LaunchVaultV7",
            LaunchVaultV7::DISCRIMINATOR,
            LaunchVaultV7::INIT_SPACE,
        ),
        (
            "TreasuryApprovalV7",
            TreasuryApprovalV7::DISCRIMINATOR,
            TreasuryApprovalV7::INIT_SPACE,
        ),
    ] {
        let a = idl["accounts"]
            .as_array()
    ...
    assert_eq!(data.len(), 8 + LaunchVaultV7::INIT_SPACE);
}
