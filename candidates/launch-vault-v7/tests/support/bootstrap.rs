...
fn preparation_is_not_a_policy_and_cannot_be_closed_updated_or_recreated() {
    ...
    assert!(f
        .run_many(
            vec![f.deposit_ix(FOUNDER, f.config.founder_amount)],
            f.creator.pubkey()
        )
        .is_err());
    ...
    for name in ["open_policy", "update_preparation", "close_preparation"] {
        ...
        attack.data = solana_sha256_hasher::hash(format!("global:{name}").as_bytes()).to_bytes()
            [..8]
            .to_vec();
    ...
    for attack in ["owner", "discriminator", "config", "bump", "address"] {
        ...
        match attack {
            ...
            "address" => {
                ...
                open.accounts[8].pubkey = address;
            }
            _ => unreachable!(),
        }
        f.svm.set_account(address, account).unwrap();
        assert!(
            f.run_many(vec![open], f.creator.pubkey()).is_err(),
            "{attack}"
        );
...
fn live_mint_authorities_and_past_t0_are_rechecked_at_both_stages() {
    for after_prepare in [false, true] {
        for attack in ["mint", "freeze", "supply", "time"] {
            ...
            let action = if after_prepare {
                f.open_ix()
            } else {
                f.prepare_ix()
            };
            assert!(
                f.run_many(vec![action], f.creator.pubkey()).is_err(),
                "{attack}"
            );
            assert!(f.svm.get_account(&f.policy).is_none());
        }
...
fn default_sbf_also_refuses_confirmation_of_a_valid_preparation() {
    ...
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../target/v7-disabled/launch_vault_v7.so");
...
fn signed_message_and_each_signature_bind_preparation_program_and_policy() {
    ...
    for index in 0..6 {
        ...
        assert!(
            format!("{:?}", f.svm.send_transaction(missing).unwrap_err())
                .contains("SignatureFailure")
        );
    }
    for index in [8, 9] {
        ...
        assert!(
            format!("{:?}", f.svm.send_transaction(changed).unwrap_err())
                .contains("SignatureFailure")
        );
    }
    let mut changed = tx.clone();
    ...
    changed.message.account_keys[key_index] = Pubkey::new_unique();
    assert!(
        format!("{:?}", f.svm.send_transaction(changed).unwrap_err()).contains("SignatureFailure")
    );
