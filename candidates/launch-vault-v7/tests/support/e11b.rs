//! E-11B signed native-loader/SPL setup and beneficiary recovery continuity.
...
pub(super) fn prepare(
    ...
    let target = Keypair::new_from_array([89; 32]);
    assert_eq!(target.pubkey(), ID);
    deploy(f, &target, &artifact("v7-test/launch_vault_v7.so"), true);
    f.run_extra(
        vec![
...
fn snapshot(
    ...
    serde_json::json!({"schema":"K4V-LAUNCH-V7-RAW-SNAPSHOT-v1","program_id":ID.to_string(),
...
fn signed_native_loader_dual_withdrawal_recovery_preserves_money_notices_and_year_boundary() {
    ...
    let new_f = key("e11b-founder-successor-symbolic-test-key");
    let new_t = key("e11b-treasury-successor-symbolic-test-key");
    ...
    for (role, successor) in [(FOUNDER, &new_f), (TREASURY, &new_t)] {
        ...
        f.run_extra_delayed(vec![instruction], f.outsider.pubkey(), &[successor], 30)
            .unwrap();
        let q: WithdrawalProposalV7 = f.read(f.withdrawal_proposal(role, 1));
        assert_eq!(
    ...
    f.time(
        f.read::<WithdrawalProposalV7>(f.withdrawal_proposal(FOUNDER, 1))
            .execute_after
            - 1,
    );
    f.reject_unchanged(
    ...
    f.time(
        f.read::<WithdrawalProposalV7>(f.withdrawal_proposal(TREASURY, 1))
            .execute_after,
    );
    f.report(f.config.shared_hard_cap);
    snapshots.push(snapshot(
    ...
    assert_eq!(
        f.read::<TreasuryApprovalV7>(f.approval(9)).author,
        old_t.pubkey()
    );
    f.reject_unchanged(
    ...
    let expiry = f
        .read::<WithdrawalProposalV7>(f.withdrawal_proposal(FOUNDER, 3))
        .expires_at;
    ...
    if let Ok(path) = std::env::var("K4V_E11B_REHEARSAL_OUT") {
        let value = serde_json::json!({"schema":"K4V-E11B-LOCAL-REHEARSAL-v1",
            "scope":"AUTHOR_RUN_LOCAL_LITESVM","private_keys_serialized":false,
